...
\wwwr{http://www.tex.ac.uk/tex-archive/info/visualFAQ/visualFAQ.pdf}{A Visual FAQ for \LaTeX{}}
...
\setpartpreamble{%
  ...
  concepts of \LaTeX\ documents.
  
  This is \emph{not} a comprehensive guide to \LaTeX, but a list of
...
The layout of a normal \LaTeX\ document description is given in
\autoref{lst:anatomy}.
\begin{lstlisting}[%
  caption={The anatomy of a \LaTeX\ file},%
  label=lst:anatomy,%
  float,%
...
used for the fragments of \LaTeX\ code in this document).
...
It is the ability to make definitions that gives \LaTeX\ its real
...
optional parameter.  For these, see a standard book on \LaTeX, such as
those listed in \autoref{sec:books}.\footnote{The underlying \TeX\ 
...
\begin{lstlisting}[
  float,
  caption={Markup for the Otway-Rees protocol \citep{OtwayRees1987}},
  label={lst:OtwayRees}
  ]
  \begin{protocol}
...
Sometimes it is convenient to break a document into pieces.  \LaTeX\
...
\LaTeX\ text is processed in various \emph{modes}.  The same input
...
\end{description}
It is rare to be caught out by the wrong mode, as \LaTeX\ usually
...
text mode that only makes sense in math mode, when \LaTeX\ will report
an error.  The reverse mistake ---to place text in a math mode---
results in ugly output; see \autoref{fig:textasmath}.
\begin{figure}[tbp]
  \begin{center}
    \newcommand*{\phrase}{%
...
\item `\lstinline|\ |' A hard space.  It may be replaced by an
  ...
  Their other use is to prevent \LaTeX\ from thinking it is at a
  sentence end when it is not.  \LaTeX\ (because \TeX\ does) treats a
...
\LaTeX\ has three kinds of lists available:
\begin{description}
...
for the label (which \emph{must} be present).  \LaTeX\ changes the
...
format.  This is a flat text file.  The standard references
(\autoref{sec:books}) will tell you how to format this file.  The
references for this document are an example.  Each entry has the
following layout:
\begin{quote}
  \sffamily
  \begin{tabular}{l}
...
\item Multiple authors in an author field must be separated with
  ...
  \end{quote}
  Commas \emph{must not} be used to separate names as they are used to
  indicate a surname occurring before the forename.
\item Capital letters that must stay as capital letters should be
  protected from \BibTeX's formatting by braces:
  \begin{quote}
    \sffamily
    title = $\{$A Paper on $\{$C++$\}$: Its use with $\{$Z$\}$, $\{$B$\}$ and $\{$CSP$\}\}$
  \end{quote}
\end{enumerate}
  
Various packages enhance the presentation of bibliographies and
citations.  The \lstinline{UoYCSproject} class loads the
\lstinline{natbib} style, which provides either Harvard-style
citations (author-year) or Toronto-style (numerical) via class options
(see \autoref{sec:options}.  The \lstinline{UoYCSproject} class also
fixes the bibliography style.

Citations are of two types (the examples show them for the author-year
style).
\begin{description}
\item[Parenthesized] If \lstinline|Joyce:FW| is a label associated
  with the record for James Joyce's \emph{Finnegans Wake}, published
  in 1939, then\newline
  \begin{tabular}[t]{l@{~\textrm{generates}~}l}
    ...
    \\
    \lstinline|\citep[James][\S4]{Joyce:FW}|&\citep[James][\S4]{Joyce:FW}
  \end{tabular}

  If numerical citations are used the text `Joyce, 1939' is replaced
  by a number.
\item[Textual]
  \begin{tabular}[t]{l@{~\textrm{generates}~}l}
    ...
    \\
    \lstinline|\citet[James][\S4]{Joyce:FW}|&\citet[James][\S4]{Joyce:FW}
  \end{tabular}

  If numerical citations are used the text `1939' is replaced by a
  number.
\end{description}
...
might produce `\ldots important modernist works~[16, 27].'\ in Toronto
style.

The \lstinline|natbib| package provides several other facilities for
controlling citation format; see its documentation.
...
Each float can have a symbolic label for use by \LaTeX's
...
\LaTeX\ allows you to define special series of named and numbered
...
\LaTeX\ has superb type-setting facilities for mathematics (see
...
possibly need.  Most people only need the basic, pre-loaded \LaTeX{}
facilities.\footnote{There is a school of thought that mistakes were
  made in the basic, pre-loaded \LaTeX{} facilities, particularly the
...
\LaTeX\ has a powerful cross-reference mechanism.  Anything for which
...
\begin{lstlisting}[
  ...
  \section[Brief titles]{How to avoid tedious prolixity in the titles of sections}
  \label{sec:brief}
\end{lstlisting}
...
Pictures in \LaTeX\ can either be
\begin{itemize}
\item drawn within \LaTeX's native \lstinline|picture| environment (if
  they are simple)
\item drawn by a more sophisticated package, such as
  \lstinline|pgf| and \lstinline|pdftricks|, or
...
The \LaTeX\ class \lstinline|UoYCSproject| is based on the
...
\lstinline{plainnat}.  The options passed to \lstinline{natbib} depend
on the class option chosen (see \autoref{sec:options}).
...
There is a choice of class options, to control citation style:
\begin{description}
\item[authoryearcitations] generates citations in the style `(author
  name, year)', and
\item[numericalcitations] generates citations in the style `[number]'.
\end{description}
Exactly one of these options must be given
(use
\begin{quote}
  \lstinline|\documentclass[authoryearcitations]{\UoYCSproject}|
\end{quote}
 or
 \begin{quote}
   \lstinline|\documentclass[numericalcitations]{\UoYCSproject}|
 \end{quote}
).
...
report.\footnote{Under Un*x you can do this by running
  \lstinline/wc -w/ on the file.  If you split the document between
  files you can use a command of the pattern \lstinline/cat file1
  file2 file3 | wc -w/.}  Optionally you can generate text that states
which extra sections are included, and which excluded by the
\lstinline|\includes| and \lstinline|\excludes| declarations.  If both
optional declarations are omitted the message produced is:
\begin{quote}
...
\item[gloss] To aid production of a glossary.
...
\item[listings] To pretty-print code listings.  Several languages are
  ...
  WARNING: the version of \lstinline{listings} on the
  \href{http://www.tug.org/texlive/}{\TeX-Live~2007} distribution is
  broken; Version~1.4 is known to work (see
...
\item[pdfpages] Allows you to include a PDF document inside your
  \LaTeX\ document.  It is very flexible, allowing you to select
  pages, print \emph{n} logical pages per physical page, and so on.
\item[pgf/tikz] Allows more complex drawings than native \LaTeX{}
  mode.  Suitable for all flavours of \LaTeX{}.  There is a very well
  deisigned font end to \lstinline{pgf}, called \lstinline{TikZ}, that
  makes drawing diagrams much easier.
\item[pict2e] An improved version of the native \LaTeX{} picture mode.
  Fewer restrictions on commands, and more flexible.
...
You may need to give a presentation, and there are several \LaTeX{}
packages for preparing slides; a good list may be found at
\url{http://www.miwie.org/presentations/}.

The main advantage of using a \LaTeX{}-based solution for
presentations is being able to re-use your \LaTeX{} source and avoid
re-typing everything for \lstinline|PowerPoint| (or similar
presentation tool, such as the one in \lstinline|OpenOffice|).  The
packages \lstinline|beamer| and \lstinline|powerdot| are very good;
the latter having the neater documentation.  Section~5 of the Beamer
\emph{User's Guide} gives a lot of good advice on creating
presentations, even (or especially) if you opt to use
\lstinline|PowerPoint|.

\cleardoublepage
\chapter{Common \LaTeX\ `Gotchas'}
\label{cha:gotchas}

There are just a few things that trip up a newcomer to \TeX\ and
\LaTeX.  Here are four.
...
problem most commonly arises with abbreviations such as `etc.',
`i.e.', `e.g.' and so on.\footnote{Some people think it is better
  style to use `and so on', `that is' and `for example', and so on,
  neatly avoiding the problem; it also avoids the quite common
  confusion between `i.e.' and `e.g.'.}
...
There are four different types of dash available in \TeX-based systems
--- systems that derive from the capabilities of typewriters and ASCII
often only have one --- the different dashes have different uses: see
...
To run \LaTeX{} from the command line on a file called
...
should run `\lstinline|bibtex foo|' \emph{after} running \LaTeX{} ---
